...
// then truncates the output by stripping all consecutive null bytes from the right,
// dumps the resulting decrypted plaintext (if decryption succeeded), and finally copies the data
...
void my_decrypt(void *p1, int p2, void *buf, unsigned long long len, int *err)
{
    ...
    // Call the original decryption function into a temporary buffer.
    ...
    // --- Truncate buffer by stripping trailing null bytes ---
    int effectiveLen = decryptedLen;
    while (effectiveLen > 0 && ((char *)tempBuffer)[effectiveLen - 1] == '\0')
    {
        effectiveLen--;
    }
    printf("[my_decrypt] Truncated decrypted data length: from %d to %d\n", decryptedLen, effectiveLen);

    if (effectiveLen > 0)
    {
        printf("[my_decrypt] Decryption succeeded. Dumping %d bytes of data.\n", effectiveLen);
        dump_message((const char *)tempBuffer, effectiveLen);
    }
    else
    {
        printf("[my_decrypt] Decryption failed or produced zero-length output after truncation.\n");
    }

    memcpy(buf, tempBuffer, effectiveLen);
    free(tempBuffer);
}
