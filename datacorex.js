...
// Target: Function sub_14721A870 in module StarCitizen.exe at IDA offset 0x721a870
// Platform: Linux
...
(function () {
    ...
    function hookImplementation(moduleBase) {
        ...
        console.log(`[IDAFrida] Attempting to hook sub_14721A870 at address ${targetAddress} (offset ${relativeOffset} from ${moduleBase})`);

        try {
            Interceptor.attach(targetAddress, {
                onEnter: function (args) {
                    this.log = [`[IDAFrida] ENTER sub_14721A870 @ ${targetAddress} (offset ${relativeOffset})`];
                    this.params = [];
                    const numArgs = parseInt("1", 10);
                ...
                onLeave: function (retval) {
                    const log = [`[IDAFrida] LEAVE sub_14721A870 @ ${targetAddress} (offset ${relativeOffset})`];
                    // @ts-ignore
                    log.push(`  retval: ${formatArgument(retval)}`);
                    console.log(log.join('\n'));
                }
            });
            console.log(`[IDAFrida] Successfully hooked sub_14721A870 at ${targetAddress}`);
        } catch (error) {
            // @ts-ignore
            console.error(`[IDAFrida] Failed to hook sub_14721A870 at ${targetAddress}: ${error.message}`);
            // @ts-ignore
