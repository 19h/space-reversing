...
class StarCitizenNavigationApp {
    ...
    public navigate(destinationName: string): void {
        ...
        try {
            ...
            // Determine the solar system from coordinates
            const currentSystem = this.planner.determineCurrentSolarSystem();
            ...
            if (!this.renderer) {
                ...
                if (this.canvasElement) {
                    ...
                    this.renderer = new EnhancedNavigationHUD(
                        this.canvasElement,
                        plan,
                        objectContainers // Pass all containers, filtering will be done internally
                    );
                    
                    // Explicitly trigger solar system render
...
/**
 * Initialize the navigation application and perform test navigation
 */
function initNavThreeJS(): void {
    ...
    // Using deterministic local coordinates instead of global ones
...
try {
    ...
    console.error("Fatal error initializing navigation application:", error);
    ...
    errorElement.textContent = `Navigation system initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
