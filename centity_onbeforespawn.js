...
// Target: Function public: bool CEntitySystem::OnBeforeSpawn in module StarCitizen.exe at IDA offset 0x7861bc0
// Platform: Linux
...
(function () {
    ...
    const TARGET_OFFSET = ptr("0x7861bc0"); // Offset relative to IDA's imagebase
    ...
    function hookImplementation(moduleBase) {
        ...
        console.log(`[IDAFrida] Attempting to hook public: bool CEntitySystem::OnBeforeSpawn at address ${targetAddress} (offset ${relativeOffset} from ${moduleBase})`);

        try {
            Interceptor.attach(targetAddress, {
                onEnter: function (args) {
                    this.log = [`[IDAFrida] ENTER public: bool CEntitySystem::OnBeforeSpawn @ ${targetAddress} (offset ${relativeOffset})`];
                    this.params = [];
                    const numArgs = parseInt("2", 10);
                ...
                onLeave: function (retval) {
                    const log = [`[IDAFrida] LEAVE public: bool CEntitySystem::OnBeforeSpawn @ ${targetAddress} (offset ${relativeOffset})`];
                    // @ts-ignore
                    log.push(`  retval: ${formatArgument(retval)}`);
                    console.log(log.join('\n'));
                }
            });
            console.log(`[IDAFrida] Successfully hooked public: bool CEntitySystem::OnBeforeSpawn at ${targetAddress}`);
        } catch (error) {
            // @ts-ignore
            console.error(`[IDAFrida] Failed to hook public: bool CEntitySystem::OnBeforeSpawn at ${targetAddress}: ${error.message}`);
            // @ts-ignore
