...
impl CoordinateTransformer {
    ...
    pub fn transform_coordinates(
        ...
        // Calculate current rotation, no rotation if velocity is zero
        let current_rotation_degrees = if container.rot_vel.x == 0.0 {
            0.0
        } else {
            let rot_vel_x = container.rot_vel.x;
            ...
            current_rotation_fraction * 360.0
        };
        ...
        let result = match direction {
            TransformDirection::ToLocal => {
                ...
                Vector3::new(rotated.x / 1000.0, rotated.y / 1000.0, rotated.z / 1000.0)
            }
            TransformDirection::ToGlobal => {
                let scaled = Vector3::new(coords.x * 1000.0, coords.y * 1000.0, coords.z * 1000.0);
...
mod tests {
    ...
    impl MockCoordinateTransformer {
        ...
        fn transform_coordinates(
            ...
            let rot_vel_x = if container.rot_vel.x == 0.0 {
                log::warn!(
                    "Container {} has zero rotation velocity. Using dummy value.",
                    container.name
                );
    ...
    fn test_transform_coordinates_roundtrip() {
        ...
        assert!(
            (local_again.x - original_local.x).abs() < 0.001,
            "X coordinate roundtrip failed: {} vs {}",
            local_again.x,
            original_local.x
        );
        assert!(
            (local_again.y - original_local.y).abs() < 0.001,
            "Y coordinate roundtrip failed: {} vs {}",
            local_again.y,
            original_local.y
        );
        assert!(
            (local_again.z - original_local.z).abs() < 0.001,
            "Z coordinate roundtrip failed: {} vs {}",
            local_again.z,
            original_local.z
        );
    }
