...
mod container_type_tests {
    ...
    fn test_container_type_from_str() {
        assert_eq!(
            ContainerType::from_str("JumpPoint"),
            Some(ContainerType::JumpPoint)
        );
        assert_eq!(
            ContainerType::from_str("Lagrange"),
            Some(ContainerType::Lagrange)
        );
        assert_eq!(ContainerType::from_str("Moon"), Some(ContainerType::Moon));
        assert_eq!(
            ContainerType::from_str("NavalStation"),
            Some(ContainerType::NavalStation)
        );
        assert_eq!(
            ContainerType::from_str("Planet"),
            Some(ContainerType::Planet)
        );
        assert_eq!(
            ContainerType::from_str("RefineryStation"),
            Some(ContainerType::RefineryStation)
        );
        assert_eq!(
            ContainerType::from_str("RestStop"),
            Some(ContainerType::RestStop)
        );
...
mod poi_type_tests {
    ...
    fn test_poi_type_from_str() {
        ...
        assert_eq!(
            PoiType::from_str("AsteroidBelt"),
            Some(PoiType::AsteroidBelt)
        );
...
mod enum_display_tests {
    ...
    fn test_entity_type_from_str() {
        assert_eq!(
            EntityType::from_str("poi"),
            Some(EntityType::PointOfInterest)
        );
        assert_eq!(
            EntityType::from_str("container"),
            Some(EntityType::ObjectContainer)
        );
...
mod visibility_edge_tests {
    ...
    fn test_visibility_edge_creation() {
        ...
        assert_eq!(
            blocked_edge.obstruction.as_ref().unwrap().name,
            "Blocking Planet"
        );
    }
}
