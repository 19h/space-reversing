...
description:
  ...
  2. Menu items under "Edit -> Plugins" for dumping code or generating DOT graphs for multiple functions.
...
FINAL VERSION: Uses PLUGIN_HIDE for manual menu integration. Supports DOT graph generation with advanced heuristics.
"""
...
from collections import defaultdict # For graph edges
...
ACTION_LABEL_MULTI = "Dump Multiple Functions..."
...
def find_callers_recursive(target_ea, current_depth, max_depth, visited_eas, edges=None):
    """
    ...
    while ref_ea != idaapi.BADADDR:
        ...
        if caller_func:
            ...
            if caller_ea not in visited_eas:
                ...
                callers.update(find_callers_recursive(caller_ea, current_depth + 1, max_depth, visited_eas, edges=edges))
...
def detect_jump_tables(ea):
    ...
    if si:
        ...
        for i in range(si.get_jtable_size()):
            target = si.get_jtable_element(i)
            if ida_funcs.get_func(target):
                cases.append(target)
        return cases
    else:
        # Manual pattern for jmp [table + reg*scale]
        op_type = idc.get_operand_type(ea, 0)
...
def resolve_virtual_calls(target_ea, edges, vtables):
    """
        ...
        if xref.type == ida_xref.fl_CN:  # Call
            ...
            while next_ea < ida_funcs.get_func(call_ea).end_ea:
                ...
                if mnem == 'call' and idc.get_operand_type(next_ea, 0) == idaapi.o_reg:
                    # Assume rax
                    edges[target_ea].add(next_ea)  # Placeholder; resolve DLL export if string arg
                next_ea = idc.next_head(next_ea)

def find_callees_recursive(target_ea, current_depth, max_depth, visited_eas, edges=None):
    """
    ...
    resolve_virtual_calls(target_ea, edges, vtables) if edges else None
    ...
    while current_item_ea < func.end_ea and current_item_ea != idaapi.BADADDR:
        ...
        if bb:
            ...
            for itgt in indirect_targets:
                if edges is not None:
                    edges[target_ea].add(itgt)
                if itgt not in visited_eas:
                    callees_and_refs.add(itgt)
                    recursive_results = find_callees_recursive(itgt, current_depth + 1, max_depth, visited_eas, edges)
        ...
        for jtt in jt_targets:
            if edges is not None:
                edges[target_ea].add(jtt)
            if jtt not in visited_eas:
                callees_and_refs.add(jtt)
                recursive_results = find_callees_recursive(jtt, current_depth + 1, max_depth, visited_eas, edges)
                callees_and_refs.update(recursive_results)
        ...
        while cref_ea != idaapi.BADADDR:
            ...
            if ref_func and ref_func.start_ea == cref_ea:
                if edges is not None:
                    edges[target_ea].add(cref_ea)
                if cref_ea not in visited_eas:
                    callees_and_refs.add(cref_ea)
                    recursive_results = find_callees_recursive(cref_ea, current_depth + 1, max_depth, visited_eas, edges)
                    callees_and_refs.update(recursive_results)
        ...
        while dref_ea != idaapi.BADADDR:
            ...
            if ref_func and ref_func.start_ea == dref_ea:
                if edges is not None:
                    edges[target_ea].add(dref_ea)
                if dref_ea not in visited_eas:
                    callees_and_refs.add(dref_ea)
                    recursive_results = find_callees_recursive(dref_ea, current_depth + 1, max_depth, visited_eas, edges)
                    callees_and_refs.update(recursive_results)
        ...
        for i in range(idaapi.UA_MAXOP):
            ...
            if op.type == idaapi.o_imm:
                ...
                if ref_func and ref_func.start_ea == imm_val:
                    ...
                    if edges is not None:
                        edges[target_ea].add(imm_val)
                    if imm_val not in visited_eas:
                        callees_and_refs.add(imm_val)
                        recursive_results = find_callees_recursive(imm_val, current_depth + 1, max_depth, visited_eas, edges)
                        callees_and_refs.update(recursive_results)
...
def write_code_file(output_file_path, decompiled_results, start_func_eas, caller_depth, callee_depth, edges):
    ...
    try:
        ...
        with open(output_file_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"// All Included Functions:\n")
            for func_ea in sorted(decompiled_results.keys()):
                func_name = name_map.get(func_ea, f"sub_{func_ea:X}")
                f.write(f"//   - {func_name} (0x{func_ea:X})\n")
            f.write(f"// Functions Not Included: None (all found within depths are included)\n")
            f.write(f"// {'-'*60}\n\n")
            ...
            for func_ea in sorted_eas:
                ...
                # Incoming xrefs (callers)
                incoming = [fr for fr in edges if func_ea in edges[fr]]
                incoming_strs = [f"{name_map.get(fr, f'sub_{fr:X}')} (0x{fr:X})" for fr in sorted(incoming)]
                f.write(f"// Incoming xrefs for {func_name} (0x{func_ea:X}): {', '.join(incoming_strs) or 'None'}\n")

                # Outgoing xrefs (callees/refs)
                outgoing = edges[func_ea]
                outgoing_strs = [f"{name_map.get(to, f'sub_{to:X}')} (0x{to:X})" for to in sorted(outgoing)]
                f.write(f"// Outgoing xrefs for {func_name} (0x{func_ea:X}): {', '.join(outgoing_strs) or 'None'}\n")
...
def write_dot_file(output_file_path, edges, all_nodes, start_func_eas, caller_depth, callee_depth):
    ...
    try:
        ...
        with open(output_file_path, "w", encoding="utf-8") as f:
            ...
            # Edges
            for from_ea in sorted(edges):
                for to_ea in sorted(edges[from_ea]):
                    f.write(f"    \"0x{from_ea:X}\" -> \"0x{to_ea:X}\";\n")
...
def dump_task(start_func_eas, caller_depth, callee_depth, output_file_path, mode='code'):
    """
    ...
    mode: 'code' or 'graph'
    """
    global g_multi_dump_active # Not used directly here, but for consistency
    ...
    try:
        ...
        if caller_depth > 0:
            ...
            def run_find_multi_callers_main(container, visited_set_container, edges):
                combined_callers = set()
                try:
                    for start_ea in start_func_eas:
                        found = find_callers_recursive(start_ea, 1, caller_depth, visited_set_container[0], edges=edges)
            ...
            sync_status = ida_kernwin.execute_sync(
                lambda: run_find_multi_callers_main(caller_result_container, visited_caller_container, edges),
                ida_kernwin.MFF_READ
            )
        ...
        if callee_depth > 0:
            ...
            def run_find_multi_callees_main(container, visited_set_container, edges):
                combined_callees = set()
                try:
                    for start_ea in start_func_eas:
                        found = find_callees_recursive(start_ea, 1, callee_depth, visited_set_container[0], edges=edges)
            ...
            sync_status = ida_kernwin.execute_sync(
                lambda: run_find_multi_callees_main(callee_result_container, visited_callee_container, edges),
                ida_kernwin.MFF_READ
            )
        ...
        # Ensure all starts are included even if depths are 0
        all_nodes = set(start_func_eas) | visited_callers | visited_callees
...
class DumpCtxActionHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        task_thread = threading.Thread(
            target=dump_task,
            args=(set([start_func_ea]), caller_depth, callee_depth, output_file_path, 'code')
        )
        task_thread.start()
...
class DumpDotCtxActionHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        task_thread = threading.Thread(
            target=dump_task,
            args=(set([start_func_ea]), caller_depth, callee_depth, output_file_path, 'graph')
        )
        task_thread.start()
...
class DumpMultiActionHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        self.run_multi_logic('code')
        return 1

    def update(self, ctx):
        return ida_kernwin.AST_ENABLE_ALWAYS

    def run_multi_logic(self, mode):
        global g_dump_in_progress, g_multi_dump_active
        with g_dump_lock:
            if g_multi_dump_active:
        ...
        task_thread = threading.Thread(
            target=dump_task,
            args=(start_eas, caller_depth, callee_depth, output_file_path, mode)
        )
        task_thread.start()


class DumpDotMultiActionHandler(DumpMultiActionHandler):
    """Handles the activation of the multi-function DOT graph action."""
    def activate(self, ctx):
        self.run_multi_logic('graph')
        return 1
...
class CodeDumperPlugin(idaapi.plugin_t):
    ...
    help = "Right-click in Pseudocode view for single func, or use Edit->Plugins menu for multi-func operations"
    ...
    def init(self):
        ...
        # Register Multi Actions
        action_desc_multi = ida_kernwin.action_desc_t(
            ACTION_ID_MULTI, ACTION_LABEL_MULTI, DumpMultiActionHandler(), None, ACTION_TOOLTIP_MULTI, 199
        )
        if not ida_kernwin.register_action(action_desc_multi):
            print(f"CodeDumper Error: [init] Failed to register action '{ACTION_ID_MULTI}'.")
            ida_kernwin.unregister_action(ACTION_ID_CTX)
        ...
        # Attach multi actions to main menu
        print("CodeDumper: [init] Attaching multi actions to Edit/Plugins menu...")
        ida_kernwin.attach_action_to_menu("Edit/Plugins/", ACTION_ID_MULTI, ida_kernwin.SETMENU_APP)
        ida_kernwin.attach_action_to_menu("Edit/Plugins/", ACTION_ID_DOT_MULTI, ida_kernwin.SETMENU_APP)
    ...
    def run(self, arg):
        pass  # Not used since PLUGIN_HIDE
